...
const collateUserData = (data) => {
    const collatedUsers = []; 
        ...
        // Checking if user exists in our new JSON
         const index = collatedUsers.findIndex(element => element.user_id === current_entry.user_id);
        //  If it doesn't exist then create it as a new entry
         if (index === -1) {
             collatedUsers.push({user_id: current_entry.user_id, job_ids: [current_entry.job_id]})
         } 
...
for (let i = 0; i < data.length; i++) {
    for(let j = i+1; j < data.length; j++) {
        ...
        const intersection = userA.job_ids.filter(value => userB.job_ids.includes(value))
