...
const collateCompanyData = (companies) => {
  ...
  for (let i = 0; i < companies.length; i++) {
    ...
    const index = collatedCompanies.findIndex(
      (element) => element.company_id === currentEntry.company_id
    );
    //  If it doesn't exist then create it as a new entry
    if (index === -1) {
      collatedCompanies.push({
        company_id: currentEntry.company_id,
        job_ids: [currentEntry.job_id],
      });
    }
  ...
  for (let i = 0; i < companies.length; i++) {
    for (let j = i + 1; j < companies.length; j++) {
      ...
      for (let k = 0; k < users.length; k++) {
        ...
        const company1Liked = company1Jobs.some((job_id) =>
          userJobs.includes(job_id)
        );
        const company2Liked = company2Jobs.some((job_id) =>
          userJobs.includes(job_id)
        );

        // Updating similarity score
        if (company1Liked && company2Liked) {
